...
/* ══════════════════════════════════════════════════════════════════════
     ...
     Orbital Rings with Aurora-inspired colors and particle effects
  ══════════════════════════════════════════════════════════════════════ */
  function sceneHero(canvas) {
    ...
    // Rings with aurora colors — use correct palette keys: a=accent, b=violet, d=cyan
    const ringColors = [pal.a, pal.b, pal.d];
    const rings = [6, 10, 14];
    const orbitGroups = [];
    ...
    rings.forEach((r, ri) => {
      ...
      // Secondary glow ring (slightly larger)
      const glowPts = [];
      const glowRadius = r + 0.3;
      for (let i = 0; i <= segments; i++) {
        const a = (i / segments) * Math.PI * 2;
        glowPts.push(new THREE.Vector3(Math.cos(a) * glowRadius, Math.sin(a) * glowRadius, 0));
      }
      ...
      // Dots on ring with varying sizes
      const count = 8 + ri * 6;
      ...
      for (let i = 0; i < count; i++) {
        ...
        const dotMat = new THREE.MeshBasicMaterial({ 
          color: ringColors[ri], 
          transparent: true, 
          opacity: 0.5 + 0.3 * Math.sin(i * 1.5)
        });
        const dot = new THREE.Mesh(dotGeo, dotMat);
        const sizeVariation = 0.8 + 0.4 * Math.sin(i * 0.7);
        dot.scale.set(sizeVariation, sizeVariation, sizeVariation);
        dot.position.set(Math.cos(a) * r, Math.sin(a) * r, 0);
        og.add(dot);
      }
      
      // Tilt each ring differently for dynamic look
      og.rotation.x = ri * 0.6 + 0.3;
      og.rotation.z = ri * 0.4;
    });

    // Center sphere with pulse
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      particlePositions[i * 3 + 2] = Math.sin(theta) * Math.sin(phi) * radius;
      
      const color = new THREE.Color([pal.a, pal.b, pal.d][Math.floor(Math.random() * 3)]);
    ...
    function animate(time) {
      ...
      // Rotate rings with different speeds and directions
      orbitGroups[0].rotation.y = t * 0.6 + (mx - 0.5) * 0.3;
      orbitGroups[1].rotation.y = -t * 0.4 + (mx - 0.5) * 0.2;
      orbitGroups[2].rotation.y = t * 0.25 + (mx - 0.5) * 0.15;
      
      // Center sphere rotation
      center.rotation.y = t * 0.8;
      center.rotation.x = Math.sin(t * 0.2) * 0.2;
      
      // Inner glow rotation
      innerGlow.rotation.y = -t * 0.5;
      innerGlow.rotation.x = Math.cos(t * 0.15) * 0.3;
      
      // Pulse center sphere
      const pulse = 1 + 0.1 * Math.sin(t * 0.5);
      center.scale.set(pulse, pulse, pulse);
      innerGlow.scale.set(1 + 0.05 * Math.sin(t * 0.3 + 1), 1 + 0.05 * Math.sin(t * 0.3 + 1), 1 + 0.05 * Math.sin(t * 0.3 + 1));
      
      // Entire group rotation
      group.rotation.y = t * 0.08 + (mx - 0.5) * 0.5;
      group.rotation.x = Math.sin(t * 0.03) * 0.1 + (my - 0.5) * 0.3;
      
      // Animate particles
      const positions = particles.geometry.attributes.position.array;
      for (let i = 0; i < particleCount; i++) {
        positions[i * 3 + 1] += Math.sin(t * 0.001 + i * 0.05) * 0.001;
        const angle = t * 0.0005 + i * 0.02;
        const radius = 5 + Math.sin(t * 0.001 + i * 0.1) * 3;
